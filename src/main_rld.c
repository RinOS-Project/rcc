...
static int parse_args(int argc, char** argv) {
    ...
    if (!g_opts.emit_unsigned_v3 &&
        ...
        fprintf(stderr, "rld: final v3 output requires --rinsign, --sign-key and --public-key\n");
        return -1;
    }
...
int main(int argc, char** argv) {
    ...
    if (!g_opts.emit_unsigned_v3) {
        if (snprintf(unsigned_path, sizeof(unsigned_path), "%s.rld-unsigned.tmp",
                     g_linker_opts.output_file) >= (int)sizeof(unsigned_path)) {
            fprintf(stderr, "rld: output path is too long for signing stage\n");
            return 1;
        }
