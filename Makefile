...
test-block-extern: $(RCC_TARGET) $(RLD_TARGET) $(RINVALIDATE)
	...
	@if $(RCC_TARGET) --target x86_64-unknown-rinos -c \
		-o $(TEST_OUT)/block-extern/invalid.ro \
		tests/invalid_block_extern.c \
		>$(TEST_OUT)/block-extern/invalid.log 2>&1; then \
		echo "invalid block-scope extern initializer unexpectedly compiled"; exit 1; \
	fi
	grep -q "block-scope extern declaration cannot have an initializer" \
		$(TEST_OUT)/block-extern/invalid.log
