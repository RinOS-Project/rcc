...
BOOTSTRAP_RCC_OBJECTS = utils lexer parser ast symtab sema codegen codegen64 \
                        preproc ir ir_pass mir mir_alloc mir_phi x86_abi x86_select ir_lower optimize \
...
test-ir:
	...
	$(CC) -m32 $(CFLAGS) -I$(INCDIR) -o $(TEST_OUT)/mir_test-x86 \
		...
		$(SRCDIR)/mir_alloc.c $(SRCDIR)/mir_phi.c \
		$(SRCDIR)/x86_abi.c $(SRCDIR)/x86_select.c $(SRCDIR)/utils.c
...
$(OBJDIR)/ir_lower.o: $(INCDIR)/rcc.h $(INCDIR)/ast.h $(INCDIR)/ir.h $(INCDIR)/ir_pass.h $(INCDIR)/mir.h $(INCDIR)/mir_alloc.h $(INCDIR)/mir_phi.h $(INCDIR)/x86_abi.h $(INCDIR)/x86_select.h $(INCDIR)/ir_lower.h
